    ...
    <div className="bg-white rounded-2xl shadow-xl p-8">
        ...
        <div className="flex flex-col gap-4 justify-center items-center">
            ...
            {isPlaying ? (
              <div className="flex items-center space-x-3">
                <div className="flex space-x-1">
                  ...
                  <div
                    className="w-2 h-5 bg-white rounded-full animate-bounce"
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-gray-500">
              {isPlaying && selectedNote
                ? `Now playing: ${selectedNote.name}`
                : "Tap to play the chromatic scale"}
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="mb-6 bg-blue-50 rounded-xl p-4 border border-blue-200">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-blue-800">
              Progress:{" "}
              {NOTES.findIndex((note) => note.name === selectedNote?.name) +
                1 || 0}
              /{NOTES.length}
            </span>
          ...
          <div className="w-full bg-blue-200 rounded-full h-2">
            <div
              ...
              style={{
                width: `${
                  ((NOTES.findIndex(
                    (note) => note.name === selectedNote?.name
                  ) +
                    1) /
                    NOTES.length) *
                  100
        ...
        <div className="flex items-start space-x-2">
          ...
          <div>
            ...
            <p className="text-yellow-700 text-xs">
              Ensure your device is not on silent/vibrate mode and volume is
              turned up for best experience.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-6 bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl p-6 border border-purple-200">
        ...
        <ul className="text-gray-600 space-y-2 text-sm">
          ...
          <li>
            • Notice how sharps have a slightly brighter, more tense sound
          </li>
        </ul>
      </div>
    </div>
